import React from 'react';
import styled from 'styled-components';
import ffssLogo from '../img/ffss_LOGO.png';
import ffssLogoHead from '../img/ffss_LOGO.svg';
import help from '../img/help.svg';
import profileUser from '../img/user.svg';
import card from '../img/card.svg';
import ffssLogoTitle from '../img/ffssLogoTitle.png';
import home from '../img/home.svg';
import payment from '../img/payment.svg';
import calculator from '../img/calculator.svg';
import account from '../img/account.svg';
import cart from '../img/cart.svg';
import backspace from '../img/backspace.svg';
import search from '../img/search.svg';
import translate from '../img/translate.svg';
import arrow_left from '../img/arrow_left.svg';
import arrow_right from '../img/arrow_right.svg';
import arrow_downward from '../img/arrow_downward.svg';
import arrow_drop_down from '../img/arrow_drop_down.svg';
import arrow_drop_up from '../img/arrow_drop_up.svg';

export const ArrowDropDown = styled.img.attrs({
  src: arrow_drop_down,
  alt: 'arrow_drop_down',
  title: 'arrow_drop_down',
  viewBox: '0 0 24 24',
})`
  width: ${(props) => props.size || 2}rem;
  height: auto;
`;
export const ArrowDropUp = styled.img.attrs({
  src: arrow_drop_up,
  alt: 'arrow_drop_up',
  title: 'arrow_drop_up',
  viewBox: '0 0 24 24',
})`
  width: ${(props) => props.size || 2}rem;
  height: auto;
`;

export const ArrowDownward = styled.img.attrs({
  src: arrow_downward,
  alt: 'arrow_downward',
  title: 'arrow_downward',
  viewBox: '0 0 24 24',
})`
  width: ${(props) => props.size || 2}rem;
  height: auto;
`;

export const ArrowLeft = styled.img.attrs({
  src: arrow_left,
  alt: 'arrow_left',
  title: 'arrow_left',
  viewBox: '0 0 24 24',
})`
  width: ${(props) => props.size || 2}rem;
  height: auto;
`;

export const ArrowRight = styled.img.attrs({
  src: arrow_right,
  alt: 'arrow_right',
  title: 'arrow_right',
  viewBox: '0 0 24 24',
})`
  width: ${(props) => props.size || 2}rem;
  height: auto;
`;

export const TranslateIcon = styled.img.attrs({
  src: translate,
  alt: 'translate',
  title: 'translate',
  viewBox: '0 0 24 24',
})`
  width: ${(props) => props.size || 2}rem;
  height: auto;
`;

// top Mobile navigation icons

export const BackspaceIcon = styled.img.attrs({
  src: backspace,
  alt: 'backspace',
  title: 'backspace',
  viewBox: '0 0 24 24',
})`
  width: ${(props) => props.size || 2}rem;
  height: auto;
`;
export const SearchIcon = styled.img.attrs({
  src: search,
  alt: 'search',
  title: 'search',
  viewBox: '0 0 24 24',
})`
  width: ${(props) => props.size || 2}rem;
  height: auto;
`;
// end of top Mobile navigation icons

// bottom Mobile navigation icons
export const HomeIcon = styled.img.attrs({
  src: home,
  alt: 'home',
  title: 'home',
  viewBox: '0 0 24 24',
})`
  width: ${(props) => props.size || 2}rem;
  height: auto;
`;

export const OrderIcon = styled.img.attrs({
  src: payment,
  alt: 'order',
  title: 'payment',
  viewBox: '0 0 24 24',
})`
  width: ${(props) => props.size || 2}rem;
  height: auto;
`;

export const CalculatorIcon = styled.img.attrs({
  src: calculator,
  alt: 'calculator',
  title: 'calculator',
  viewBox: '0 0 24 24',
})`
  width: ${(props) => props.size || 2}rem;
  height: auto;
`;

export const HelpIcon = styled.img.attrs({
  src: help,
  alt: 'help',
  title: 'help',
  viewBox: '0 0 24 24',
})`
  width: ${(props) => props.size || 2}rem;
  height: auto;
`;

export const AccountIcon = styled.img.attrs({
  src: account,
  alt: 'account',
  title: 'account',
  viewBox: '0 0 24 24',
})`
  width: ${(props) => props.size || 2}rem;
  height: auto;
`;

export const CartIcon = styled.img.attrs({
  src: cart,
  alt: 'cart',
  title: 'cart',
  viewBox: '0 0 24 24',
})`
  width: ${(props) => props.size || 2}rem;
  height: auto;
`;

// end of bottom Mobile navigation icons

export const Compass = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="24"
    height="24"
    viewBox="0 0 24 24"
  >
    <path d="M12 2c5.514 0 10 4.486 10 10s-4.486 10-10 10-10-4.486-10-10 4.486-10 10-10zm0-2c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm6 5.999l-5.621 2.986c-.899-.104-1.806.191-2.474.859-.662.663-.95 1.561-.862 2.428l-3.043 5.728 5.724-3.042c.884.089 1.772-.205 2.432-.865.634-.634.969-1.524.859-2.473l2.985-5.621zm-5.97 7.22c-.689 0-1.25-.559-1.25-1.249-.001-.691.559-1.251 1.25-1.25.69 0 1.25.56 1.25 1.25-.001.689-.56 1.249-1.25 1.249z" />
  </svg>
);

export const HeartEmpty = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="24"
    height="24"
    viewBox="0 0 24 24"
  >
    <path d="M6.28 3c3.236.001 4.973 3.491 5.72 5.031.75-1.547 2.469-5.021 5.726-5.021 2.058 0 4.274 1.309 4.274 4.182 0 3.442-4.744 7.851-10 13-5.258-5.151-10-9.559-10-13 0-2.676 1.965-4.193 4.28-4.192zm.001-2c-3.183 0-6.281 2.187-6.281 6.192 0 4.661 5.57 9.427 12 15.808 6.43-6.381 12-11.147 12-15.808 0-4.011-3.097-6.182-6.274-6.182-2.204 0-4.446 1.042-5.726 3.238-1.285-2.206-3.522-3.248-5.719-3.248z" />
  </svg>
);

export const HeartFull = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="24"
    height="24"
    viewBox="0 0 24 24"
    fill="#ED4956"
  >
    <path d="M12 4.248c-3.148-5.402-12-3.825-12 2.944 0 4.661 5.571 9.427 12 15.808 6.43-6.381 12-11.147 12-15.808 0-6.792-8.875-8.306-12-2.944z" />
  </svg>
);

export const User = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="24"
    height="24"
    viewBox="0 0 24 24"
  >
    <path d="M12 2c2.757 0 5 2.243 5 5.001 0 2.756-2.243 5-5 5s-5-2.244-5-5c0-2.758 2.243-5.001 5-5.001zm0-2c-3.866 0-7 3.134-7 7.001 0 3.865 3.134 7 7 7s7-3.135 7-7c0-3.867-3.134-7.001-7-7.001zm6.369 13.353c-.497.498-1.057.931-1.658 1.302 2.872 1.874 4.378 5.083 4.972 7.346h-19.387c.572-2.29 2.058-5.503 4.973-7.358-.603-.374-1.162-.811-1.658-1.312-4.258 3.072-5.611 8.506-5.611 10.669h24c0-2.142-1.44-7.557-5.631-10.647z" />
  </svg>
);

export const Comment = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="24"
    height="24"
    viewBox="0 0 24 24"
  >
    <path d="M12 3c5.514 0 10 3.476 10 7.747 0 4.272-4.48 7.748-9.986 7.748-.62 0-1.092-.046-1.759-.097-1 .776-1.774 1.403-3.485 1.962.26-1.383-.113-2.259-.514-3.259-2.383-1.505-4.256-3.411-4.256-6.354 0-4.271 4.486-7.747 10-7.747zm0-2c-6.627 0-12 4.363-12 9.747 0 3.13 1.816 5.916 4.641 7.699.867 2.167-1.084 4.008-3.143 4.502 3.085.266 6.776-.481 9.374-2.497 7.08.54 13.128-3.988 13.128-9.704 0-5.384-5.373-9.747-12-9.747z" />
  </svg>
);

export const CommentFull = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="24"
    height="24"
    viewBox="0 0 24 24"
  >
    <path d="M12 1c-6.627 0-12 4.364-12 9.749 0 3.131 1.817 5.917 4.64 7.7.868 2.167-1.083 4.008-3.142 4.503 2.271.195 6.311-.121 9.374-2.498 7.095.538 13.128-3.997 13.128-9.705 0-5.385-5.373-9.749-12-9.749z" />
  </svg>
);

export const Logo = ({ size = 24 }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width={size}
    height={size}
    viewBox="0 0 24 24"
  >
    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
  </svg>
);

export const FFssLogoTitle = styled.img.attrs({
  src: ffssLogoTitle,
  alt: 'ffssLogoTitle',
  title: 'ffssLogoTitle',
  viewBox: '0 0 24 24',
})`
  width: ${(props) => props.size || 2}rem;
  height: auto;
`;

export const FFSSlogo = styled.img.attrs({
  src: ffssLogo,
  alt: 'logo',
  title: 'logo',
  viewBox: '0 0 24 24',
})`
  width: ${(props) => props.size || 3}rem;
  height: auto;
`;

export const FFSSLogoHead = styled.img.attrs({
  src: ffssLogoHead,
  alt: 'logo',
  title: 'logo',
  viewBox: '0 0 24 24',
})`
  width: ${(props) => props.size || 3}rem;
  height: auto;
`;

export const CardLogo = styled.img.attrs({
  src: card,
  alt: 'order',
  title: 'order',
  viewBox: '0 0 24 24',
})`
  width: ${(props) => props.size || 2}rem;
  height: auto;
`;

export const ProfileUserLogo = styled.img.attrs({
  src: profileUser,
  alt: 'Profile',
  title: 'profile',
  viewBox: '0 0 24 24',
})`
  width: ${(props) => props.size || 2}rem;
  height: auto;
`;
